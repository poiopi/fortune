...
<head>
  ...
  <meta charset="UTF-8">
  <link rel="canonical" href="https://life-fun.net/articles/love/seiza/<?= $item['slug'] ?>/" />
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .sign-info-box{background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.25rem 1.5rem;margin:1.5rem 0;display:flex;gap:1.5rem;align-items:center;flex-wrap:wrap}
  .sign-symbol{font-size:2.2rem;color:var(--accent)}
  .sign-meta{font-size:.8rem;color:var(--muted);display:flex;gap:1rem;flex-wrap:wrap}
  ...
  <script type="application/ld+json">
  {
    ...
    "itemListElement": [
      ...
      {"@type":"ListItem","position":4,"name":"星座×恋愛","item":"https://life-fun.net/articles/love/seiza/"},
      {"@type":"ListItem","position":5,"name":"<?= htmlspecialchars($item['name'].'の恋愛傾向') ?>","item":"https://life-fun.net/articles/love/seiza/<?= $item['slug'] ?>/"}
    ]
  }
  </script>
</head>
...
<div class="wrap">
  ...
  <div class="art-hero">
    <span class="art-label">Seiza × Love Engine</span>
  ...
  <div class="sign-info-box">
    <div class="sign-symbol"><?= htmlspecialchars($item['symbol']) ?></div>
    <div class="sign-meta">
      <span>期間：<?= htmlspecialchars($item['period']) ?></span>
      <span>エレメント：<?= htmlspecialchars($item['element']) ?></span>
      <span>クオリティ：<?= htmlspecialchars($item['quality']) ?></span>
    </div>
  </div>

  <div class="engine-note">
    このページの「Love Engineの実測データ」は、恋愛傾向診断エンジンの9216通りの組み合わせパターンのうち、<strong><?= htmlspecialchars($item['name']) ?>に該当する<?= (int)$item['sampleSize'] ?>パターン</strong>（MBTI16×血液型4×内面タイプ12）を集計した結果です。星座の一般的な性格解説は<a href="/articles/seiza/<?= $item['slug'] ?>/" class="al-link">西洋占星術の<?= htmlspecialchars($item['name']) ?>解説</a>もあわせてご覧ください。
  </div>
  ...
  <section class="art-section" id="causal">
    ...
    <div class="causal-chain">
      ...
      <span class="causal-node"><?= htmlspecialchars($c['factor']) ?></span>
  ...
  <?php
  $prevTitle = $item['prev']['title'] ?? null;
...
<?php
$html = ob_get_clean();
echo autoLink($html, 'love-seiza-'.$item['slug']);
?>
