...
@media (max-width: 640px) { .theme-tiles { grid-template-columns: 1fr; gap: .65rem; } }
.theme-tile {
  background: linear-gradient(145deg, var(--card2), var(--card));
  border: 1px solid var(--border);
  border-radius: 14px;
...
.theme-tile::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, var(--tc1), var(--tc2)); }
.theme-tile:hover { border-color: var(--border2); transform: translateY(-3px); box-shadow: 0 10px 22px rgba(0,0,0,.26); }
.theme-tile-icon { font-size: 1.6rem; line-height: 1; }
.theme-tile-name { font-family: var(--ff-serif); font-size: .95rem; font-weight: 700; color: var(--text); }
.theme-tile-desc { font-size: .74rem; color: var(--text-secondary); line-height: 1.6; }
.theme-picker-bridge { text-align: center; font-size: .78rem; color: var(--muted); margin-top: 1.3rem; }

/* モーダル本体（.fmenu-overlay/.fmenu-drawerより上位のz-indexで新規定義。inc/footer.phpは未変更） */
.theme-modal-overlay {
  position: fixed; inset: 0; background: rgba(0,0,0,.55); backdrop-filter: blur(3px);
  z-index: 2099; opacity: 0; pointer-events: none; transition: opacity .3s;
}
.theme-modal-overlay.open { opacity: 1; pointer-events: all; }
.theme-modal {
  position: fixed; z-index: 2100;
  top: 50%; left: 50%; transform: translate(-50%, calc(-50% + 16px));
  width: min(560px, 92vw);
  max-height: 86vh; overflow-y: auto;
  background: linear-gradient(180deg, #14102a 0%, #1a1535 100%);
  border: 1px solid var(--border2);
  border-radius: 18px;
...
.theme-modal.open { opacity: 1; pointer-events: all; transform: translate(-50%, -50%); }
@media (max-width: 600px) {
  .theme-modal {
  ...
  .theme-modal.open { transform: translateY(0); }
}
.theme-modal-close {
  position: absolute; top: 1rem; right: 1rem;
  background: none; border: 1px solid rgba(155,114,239,.3); border-radius: 50%;
  width: 30px; height: 30px; color: var(--muted); cursor: pointer; font-size: .85rem;
  display: flex; align-items: center; justify-content: center;
  transition: color .2s, border-color .2s;
}
.theme-modal-close:hover { color: var(--gold-lt); border-color: var(--violet-lt); }
.theme-modal-heading { font-family: var(--ff-serif); font-size: 1.1rem; font-weight: 700; color: var(--text); text-align: center; margin: .2rem 0 1.4rem; padding-right: 1.6rem; }
.theme-date-group { display: flex; gap: .6rem; margin-bottom: 1.3rem; }
.theme-date-group select {
  flex: 1; padding: .85rem .6rem; min-height: 50px; text-align: center;
  background: var(--card); border: 1px solid var(--border2); border-radius: 8px;
  color: var(--text); font-family: var(--ff-sans); font-size: .85rem;
  appearance: none;
  background-image: linear-gradient(45deg,transparent 50%,var(--muted) 50%),linear-gradient(135deg,var(--muted) 50%,transparent 50%);
  background-position: calc(100% - 16px) center, calc(100% - 11px) center;
  background-size: 5px 5px, 5px 5px; background-repeat: no-repeat;
}
.theme-date-group select:focus { outline: none; border-color: var(--violet); box-shadow: 0 0 0 3px rgba(155,114,239,.12); }
.theme-date-group .theme-date-year { flex: 1.5; }
.theme-modal-submit { display: block; width: 100%; text-align: center; }
.theme-modal-submit:disabled { opacity: .45; cursor: not-allowed; }
.theme-modal-error { background: rgba(200,80,128,.1); border: 1px solid rgba(200,80,128,.3); border-radius: 10px; padding: 1rem; font-size: .85rem; color: var(--rose); margin-top: 1rem; }
.theme-modal-loading { text-align: center; padding: 2.6rem 0; color: var(--text-secondary); font-size: .9rem; }
.theme-modal-result {
  background: linear-gradient(145deg, var(--card2), var(--card));
  border: 1px solid var(--border); border-radius: 14px;
  padding: 1.4rem 1.2rem; position: relative; overflow: hidden;
}
.theme-modal-result::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, var(--violet), var(--gold)); }
.theme-modal-sign { font-family: var(--ff-mono); font-size: .68rem; letter-spacing: .12em; color: var(--gold); text-align: center; margin-bottom: .6rem; }
.theme-modal-text { font-size: .88rem; color: var(--text-secondary); line-height: 1.85; }
.theme-modal-more { display: flex; justify-content: center; margin-top: 1.3rem; }
.theme-modal-more a { font-family: var(--ff-mono); font-size: .7rem; letter-spacing: .08em; color: var(--gold); text-decoration: none; border: 1px solid rgba(201,168,76,.4); border-radius: 24px; padding: .5rem 1.4rem; transition: border-color .2s, background .2s; }
.theme-modal-more a:hover { border-color: var(--gold); background: rgba(201,168,76,.08); }
</style>
</head>
<body>
...
<!-- ══ 気になることから占う（相談テーマ型占いモーダル） ══ -->
<section class="theme-picker-section" id="theme-picker">
  <div class="wrap theme-picker-head">
    <span class="section-label">Pick Your Concern</span>
    <h2 class="section-title">気になることから占う</h2>
    <p class="section-sub">星座占いで、気になることのヒントを読み解きます。</p>
  </div>
  <div class="wrap">
    <div class="theme-tiles">
      <button type="button" class="theme-tile fade-up" data-theme-category="love" style="--tc1:var(--rose);--tc2:var(--violet)" data-ga-event="cta_click" data-cta-name="theme_tile_love">
        <span class="theme-tile-icon" aria-hidden="true">💗</span>
        <span class="theme-tile-name">恋愛</span>
        <span class="theme-tile-desc">恋愛スタイルがわかります</span>
      </button>
      <button type="button" class="theme-tile fade-up" data-theme-category="work" style="--tc1:var(--teal);--tc2:var(--violet)" data-ga-event="cta_click" data-cta-name="theme_tile_work">
        <span class="theme-tile-icon" aria-hidden="true">💼</span>
        <span class="theme-tile-name">仕事</span>
        <span class="theme-tile-desc">向いている仕事がわかります</span>
      </button>
      <button type="button" class="theme-tile fade-up" data-theme-category="self" style="--tc1:var(--gold);--tc2:var(--rose)" data-ga-event="cta_click" data-cta-name="theme_tile_self">
        <span class="theme-tile-icon" aria-hidden="true">✨</span>
        <span class="theme-tile-name">自分のこと</span>
        <span class="theme-tile-desc">気質・強みがわかります</span>
      </button>
    </div>
    <p class="theme-picker-bridge">占い方法をすでに決めている方は、下から選べます</p>
  </div>
</section>
...
<!-- ══ 相談テーマ型占いモーダル（気になることから占う） ══ -->
<div class="theme-modal-overlay" id="themeModalOverlay"></div>
<div class="theme-modal" id="themeModal" role="dialog" aria-modal="true" aria-labelledby="themeModalHeading">
  <button type="button" class="theme-modal-close" id="themeModalClose" aria-label="閉じる">✕</button>

  <div class="theme-modal-step" id="themeStepInput">
    <h3 class="theme-modal-heading" id="themeModalHeading"></h3>
    <div class="theme-date-group">
      <select id="themeDateYear" class="theme-date-year" aria-label="年"></select>
      <select id="themeDateMonth" aria-label="月"></select>
      <select id="themeDateDay" aria-label="日"></select>
    </div>
    <div id="themeInputError"></div>
    <button type="button" class="btn-primary theme-modal-submit" id="themeSubmitBtn" data-ga-event="fortune_submit" disabled>占ってみる</button>
  </div>

  <div class="theme-modal-step" id="themeStepLoading" hidden>
    <div class="theme-modal-loading">星座を読み解いています……</div>
  </div>

  <div class="theme-modal-step" id="themeStepResult" hidden>
    <h3 class="theme-modal-heading" id="themeResultHeading"></h3>
    <div class="theme-modal-result">
      <div class="theme-modal-sign" id="themeResultSign"></div>
      <p class="theme-modal-text" id="themeResultText"></p>
    </div>
    <div class="theme-modal-more">
      <a href="/articles/seiza/" data-ga-event="cta_click" data-cta-name="theme_fortune_to_article" data-cta-destination="/articles/seiza/">もっと詳しく見る →</a>
    </div>
  </div>
</div>
...
<script>
/* ══ 相談テーマ型占いモーダル（気になることから占う）══
   inc/birthday-input.phpは使わず独立実装（window.BirthdayInput.init()内部でtrackEvent('fortune_start', {})が
   パラメータなしで発火する副作用があり、今回はcategory付きで独自発火する必要があるため）。 */
(function(){
  var TILE_HEADINGS = {
    love: '恋愛について読み解きます',
    work: '仕事について読み解きます',
    self: '自分のことについて読み解きます'
  };
  var RESULT_HEADINGS = {
    love: 'あなたの星座から恋愛スタイルを読み解きました',
    work: 'あなたの星座から向いている仕事を読み解きました',
    self: 'あなたの星座から気質・強みを読み解きました'
  };
  ...
  var stepInput  = document.getElementById('themeStepInput');
  var stepLoading= document.getElementById('themeStepLoading');
  var stepResult = document.getElementById('themeStepResult');
  var heading    = document.getElementById('themeModalHeading');
  var yearEl     = document.getElementById('themeDateYear');
  var monthEl    = document.getElementById('themeDateMonth');
  var dayEl      = document.getElementById('themeDateDay');
  var errorBox   = document.getElementById('themeInputError');
  var submitBtn  = document.getElementById('themeSubmitBtn');
  var resultHeading = document.getElementById('themeResultHeading');
  var resultSign = document.getElementById('themeResultSign');
  var resultText = document.getElementById('themeResultText');
  ...
  function refreshDayOptions(keepValue){
    var year  = parseInt(yearEl.value, 10) || null;
    var month = parseInt(monthEl.value, 10) || null;
    var maxDay = daysInMonth(year, month);
    var current = keepValue ? parseInt(dayEl.value, 10) : null;
    ...
    if(current && current <= maxDay) dayEl.value = current;
  }
  function updateSubmitState(){
    submitBtn.disabled = !(yearEl.value && monthEl.value && dayEl.value);
  }
  function markStart(){
    if(startTracked) return;
    startTracked = true;
    if(typeof trackEvent === 'function') trackEvent('fortune_start', { category: currentCategory });
  }
  ...
  function openModal(category){
    ...
    heading.textContent = TILE_HEADINGS[category] || '';
  ...
  document.querySelectorAll('.theme-tile').forEach(function(tile){
    tile.addEventListener('click', function(){
      openModal(tile.dataset.themeCategory);
    });
  });
  ...
  submitBtn.addEventListener('click', function(){
    ...
    var birthday = yearEl.value + '-' + pad(monthEl.value) + '-' + pad(dayEl.value);
    errorBox.innerHTML = '';
    stepInput.hidden = true;
    stepLoading.hidden = false;

    fetch('/api/fortune-theme.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ category: currentCategory, birthday: birthday })
    })
    .then(function(res){ return res.json(); })
    .then(function(data){
      if(data.error) throw new Error(data.error);
      resultHeading.textContent = RESULT_HEADINGS[currentCategory] || '';
      resultSign.textContent = data.sign || '';
      resultText.textContent = data.resultText || '';
      stepLoading.hidden = true;
      stepResult.hidden = false;
      if(typeof trackEvent === 'function') trackEvent('fortune_result_view', { character: 'default' });
    })
    .catch(function(){
      stepLoading.hidden = true;
      stepInput.hidden = false;
      errorBox.innerHTML = '<div class="theme-modal-error">診断に失敗しました。もう一度お試しください。</div>';
    });
  });
})();
</script>
