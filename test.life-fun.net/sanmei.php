...
.error-box{background:rgba(232,113,154,.1);border:1px solid rgba(232,113,154,.3);border-radius:8px;padding:.75rem 1rem;margin-bottom:1rem;font-size:.88rem;color:#e8719a;display:none}
...
function calcSanmei() {
  ...
  if (!dateVal) { showInlineError('formError', '生年月日を入力してください'); return; }
  const [year, month, day] = dateVal.split('-').map(Number);
  if (year < 1900 || year > 2099) { showInlineError('formError', '1900年〜2099年の範囲で入力してください'); return; }
