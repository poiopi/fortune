...
.hero h1{font-family:var(--ff-serif);font-size:clamp(1.8rem,5vw,2.8rem);font-weight:700;line-height:1.2;letter-spacing:.06em;background:linear-gradient(135deg,var(--gold-lt) 0%,var(--violet-lt) 50%,#4ecdc4 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin-bottom:.5rem}
.today-band{background:linear-gradient(135deg,rgba(201,168,76,.12),rgba(155,114,239,.08));border:1px solid rgba(201,168,76,.25);border-radius:14px;padding:1.5rem 1.8rem;margin-bottom:2rem}
.rokuyo-desc{font-size:.82rem;color:var(--muted);line-height:1.8}
...
<?php $currentPage='privacy'; require __DIR__.'/inc/header.php'; ?>

<div class="wrap">

  <section class="hero">
    <span class="hero-eyebrow">Privacy Policy</span>
    <h1>プライバシーポリシー</h1>
  </section>
  ...
  <div class="today-band">
    <div class="rokuyo-desc">
      ...
      <strong style="color:var(--text)">個人情報の取り扱い</strong><br>
      当サイト「占いPortal」（以下、当サイト）では、三星鑑定のご利用時に入力いただくお名前（ニックネーム可）や生年月日などの情報を、鑑定結果の表示のみに使用します。これらの情報をサーバーに永続的に保存することはありません。お問い合わせの際にメールでご連絡いただいた内容については、ご返信の目的のみに使用し、適切に管理します。<br><br>

      <strong style="color:var(--text)">Google AdSenseによる広告配信</strong><br>
      当サイトでは、第三者配信の広告サービス「Google AdSense（グーグルアドセンス）」を利用しています。Googleなどの第三者広告配信事業者は、ユーザーの興味に応じた広告を表示するため、当サイトや他サイトへのアクセス情報に基づくCookie（クッキー）を使用することがあります。Cookieを使用することにより、当サイトへの過去のアクセス情報に基づいて広告が配信されます。詳しくは<a href="https://policies.google.com/technologies/ads?hl=ja" target="_blank" rel="noopener noreferrer" style="color:var(--violet-lt)">Googleの広告に関するポリシー</a>をご確認ください。<br><br>

      <strong style="color:var(--text)">Google Analyticsによるアクセス解析</strong><br>
      当サイトでは、アクセス状況の把握とサービス改善のため、Google Analytics（測定ID：G-P1EKB3WWX8）を使用しています。Google AnalyticsはCookieを使用してトラフィックデータを収集しますが、データは匿名で収集され、個人を特定するものではありません。Google Analyticsの利用規約については<a href="https://marketingplatform.google.com/about/analytics/terms/jp/" target="_blank" rel="noopener noreferrer" style="color:var(--violet-lt)">Googleのサイト</a>をご確認ください。<br><br>

      <strong style="color:var(--text)">Cookieの利用</strong><br>
      当サイトでは、広告配信・アクセス解析のほか、サービスの利便性向上のためにCookieを使用する場合があります。Cookieはお使いのブラウザの設定により無効化または削除できます。ただし、Cookieを無効にした場合、一部の機能が正常に動作しないことがあります。<br><br>

      <strong style="color:var(--text)">免責事項</strong><br>
      当サイトが提供する占い・診断の結果は、エンターテインメントを目的としたものです。医学・法律・投資・金融などの専門的な助言や判断を行うものではありません。占い結果を参考にした判断や行動について、当サイトは一切の責任を負いかねます。重要な決定は、専門家へのご相談をお勧めします。<br><br>
