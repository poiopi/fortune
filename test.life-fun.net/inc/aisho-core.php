...
// ══════════════════════════════════════════════════════════════════
// aisho.php から抽出した純粋関数群・データ（相性計算ロジック本体）
//
// aisho.php はページ単体として完結したHTML/GET処理を行うファイルであり、
// そのままrequireすると不要な出力・GETパラメータ処理が発生してしまう。
// そのため、api/fortune-chat.php（AI鑑定チャットMVP）から安全に計算ロジック
// だけを再利用できるよう、この関数群をここへ切り出した。
//
// 各データ・ロジック自体はaisho.php（4-40行目の定数・関数、291-302行目の
// 計算本体、607-608行目のラベル配列、638-645行目のmarriageScore3分岐理由文、
// 662-669行目のloverScore「魅力ポイント」データ）にあったものと完全に同一
// （一切変更していない）。marriageScore/loverScoreの計算式は一字一句変更禁止。
// confess/dateSpot/present（aisho.php 304-343行目）は今回の機能に不要のため
// 複製していない。自己完結（他ファイルへのrequireを持たない）。
// ══════════════════════════════════════════════════════════════════
...
// 複製元：aisho.php 291-302行目（計算本体）、607-608行目（ラベル）、
// 638-645行目（marriage理由文）、662-669行目（lover魅力ポイント）
function aisho_calcCompatibility(int $ym, int $yd, int $pm, int $pd): array {
    ...
    // aisho.php 294-295行目相当：1990年固定でライフパス計算（既存仕様のまま、変更しない）
    ...
    // aisho.php 637-646行目の3分岐理由文を複製。
    // 名前入力は今回のAPIに存在しないため、$yLabel/$pLabelは「あなた」「お相手」に固定する
    if ($yZ['el'] === $pZ['el']) {
