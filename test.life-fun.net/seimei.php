...
<p class="disclaimer">※ 画数は一般的な字画数（新字体）に基づいて算出しています。熊崎式など流派によって画数が異なる場合があります。<br>本サービスはエンターテインメントを目的とした占いコンテンツです。結果は楽しみや気づきの参考としてご活用ください。</p>
...
const KK={
  ...
  // 旧字体・異体字
  '邊':19,'邉':17,
  '齋':17,'齊':14,'櫻':21,'廣':15,'澁':15,'國':11,'榮':14,'眞':10,
  '壽':14,'惠':12,'萬':12,'澤':16,'瀧':19,'實':14,'豐':18,'縣':16,
  '亞':8,'應':17,'禮':17,'藝':19,
  // GA4 unknown_kanji イベント経由で追加
  '爨':29,'纛':24,'膵':15,'臍':17,'饂':17,'驫':27,'鱚':23,'鶫':20,'鸛':28,
};

function strokes(ch){
  if(KK[ch] !== undefined) return KK[ch];
  // 未登録漢字をGA4に送信して後から対応できるようにする
