...
<head>
  ...
  <meta charset="UTF-8">
  <link rel="canonical" href="https://life-fun.net/articles/love/tendency/<?= $item['slug'] ?>/" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="Love Engineの推定傾向指標「<?= htmlspecialchars($item['name']) ?>」とは何か、どう計算されるかを9216パターンの実測データで解説。High/Mid/Lowの違いとMBTI・血液型別の傾向を紹介。">
  <title><?= htmlspecialchars($item['name']) ?>（Tendency）とは｜Love Engineの計算方法と実測データを解説</title>
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .caveat-box{background:#fff8ee;border:1px solid #f0dfc0;border-radius:12px;padding:1rem 1.25rem;margin:1.5rem 0;font-size:.82rem;color:#5a4a2a;line-height:1.8}
  ...
  <script type="application/ld+json">
  {
    ...
    "itemListElement": [
      ...
      {"@type":"ListItem","position":3,"name":"推定傾向指標","item":"https://life-fun.net/articles/love/tendency/"},
      {"@type":"ListItem","position":4,"name":"<?= htmlspecialchars($item['name']) ?>とは","item":"https://life-fun.net/articles/love/tendency/<?= $item['slug'] ?>/"}
    ]
  }
  </script>
</head>
...
<div class="wrap">
  ...
  <nav class="breadcrumb">
    ...
    <a href="/articles/love/tendency/">推定傾向指標</a><span>›</span>
    <?= htmlspecialchars($item['name']) ?>とは
  </nav>

  <div class="art-hero">
    <span class="art-label">Tendency × Love Engine</span>
    <h1><?= htmlspecialchars($item['name']) ?>（Tendency）とは｜Love Engineの計算方法と実測データを解説</h1>
    <p class="art-lead"><?= htmlspecialchars($item['lead']) ?></p>
  </div>

  <div class="caveat-box">
    <strong>Tendencyについて：</strong>結婚志向・浮気耐性は、性格だけでは決定できない概念です。価値観・経験・年齢などの影響を強く受けるため、Style（積極性等）以上に断定を避けた表現で扱います（<a href="/articles/love/style/" class="al-link" style="color:#5a4a2a">Styleの記事一覧</a>もあわせてご覧ください）。
  </div>
...
<?php
$html = ob_get_clean();
echo autoLink($html, 'love-tendency-'.$item['slug']);
?>
