...
// 【比較モックアップ／案A＋C要素：改良案（a-plus）】① 問題ページ
// 案A（ライト・マガジン型）の素直さをベースに、案C（ペーパー／カード型）の
// 「選択肢バッジの色分け」だけをごく少量取り入れた改良案。
// 本番テンプレート(_quiz-tpl.php)とは独立。sitemap.xml等の変更を伴わない。
require __DIR__ . '/../_bootstrap.php';

// 4択のバッジ（a1〜a4の丸印）だけに割り当てる控えめなアクセントカラー。
// 案Cのようにカード全体（背景・枠線）を塗り分けるのではなく、
// 「小さな丸バッジ」だけに色を差すことで、Aの素直さを保ったまま
// 「4択のどれを選んでいるか」がひと目でわかる遊び心をひとつまみ加える。
$accentMap = [
    'a1' => '#c1616a', // ローズ
    'a2' => '#3f7fa6', // ブルー
    'a3' => '#3f9c74', // グリーン
    'a4' => '#b8863a', // マスタード（Aのaccentと同系）
];
?>
<!DOCTYPE html>
<html lang="ja">
...
<title>【モックアップ案A+C】<?= htmlspecialchars($quiz['question'], ENT_QUOTES, 'UTF-8') ?></title>
...
.quiz-choice:hover,.quiz-choice:focus-visible{
  ...
  /* 案Aのtranslateyに、案Cのわずかな傾き(rotate)を少量だけ加えて遊びを出す */
...
<body>
<div class="mock-badge">DESIGN MOCKUP — 案A+C（改良案・a-plus）：ライト・マガジン型＋バッジ色アクセント（noindex／本番非公開）</div>
<div class="wrap quiz-wrap" style="margin-top:2rem">
  ...
  <section class="quiz-choices">
    ...
    <a href="result.php?a=<?= $_k ?>" class="quiz-choice">
      <span class="quiz-choice-mark" style="--mark-color:<?= $accentMap[$_k] ?>"><?= strtoupper($_k) ?></span>
      <span class="quiz-choice-text"><?= htmlspecialchars(quizmock_short_label($quiz['choices'][$_k]), ENT_QUOTES, 'UTF-8') ?></span>
  ...
  <div class="mock-note">
    ...
    選択肢をクリックすると同案の回答ページ（result.php）へ遷移します。<br>
    案Aとの違い：①選択肢の丸バッジ（A/B/C/D相当）にのみ4色の控えめなアクセントカラーを追加。カード自体の背景・枠線は案Aのまま変更していません。②ホバー時に丸バッジがわずかに拡大・回転する遊びを追加。③選択肢の表示を「その人の『目』」→「目」のように簡略化。
  </div>
