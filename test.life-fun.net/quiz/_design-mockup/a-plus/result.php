...
$accentColor = $accentMap[$quizChoice];
?>
<!DOCTYPE html>
<html lang="ja">
<head>
...
<title>【モックアップ案A+C・結果】<?= htmlspecialchars($quiz['question'], ENT_QUOTES, 'UTF-8') ?></title>
...
<div class="wrap quiz-wrap" style="margin-top:2rem">
  ...
  <section class="quiz-result-block">
    <p class="quiz-your-choice">
      <span class="quiz-your-choice-mark"><?= strtoupper($quizChoice) ?></span>
      あなたが選んだのは「<b><?= htmlspecialchars($shortLabel, ENT_QUOTES, 'UTF-8') ?></b>」でした。
    </p>
  ...
  <div class="mock-note">
    ...
    「あなたが選んだのは『<?= htmlspecialchars($shortLabel, ENT_QUOTES, 'UTF-8') ?>』でした。」に短縮表示しています（表示方法の実験、恒久実装は別途検討）。<br>
    案Aとの違い：選んだ選択肢の記号（A〜D相当）に、問題ページと同じ控えめなアクセントカラーの丸バッジを添えています。
  </div>
