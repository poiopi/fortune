...
//   $quiz       : <slug>/data.php が return する配列（series==='B'）
//   $quizChoice : 'a1'〜'a4' のいずれか、または null（explanation_indexable=true かつ ?aなしの正規版のみ null 可）

if (!isset($quiz) || !array_key_exists('quizChoice', get_defined_vars())) {
    http_response_code(500);
    exit('quiz kaisetsu-b template: missing required variables');
}

$_isIndexable = !empty($quiz['explanation_indexable']);
$_hasChoice   = $quizChoice !== null;
$_numberStr   = str_pad((string)$quiz['number'], 3, '0', STR_PAD_LEFT);
$_canonicalBase = 'https://life-fun.net/quiz/' . $quiz['slug'] . '/kaisetsu/';

if ($_isIndexable) {
    // 正規版（?aなし）に常にcanonicalを向ける。indexされるのは?aなし版のみ。
    $_canonical    = $_canonicalBase;
    $_robotsNoindex = $_hasChoice;
} else {
    // indexable=falseの場合は自己canonical（常に?aあり。バリデーション済みのため必ず choice がある）
    $_canonical    = $_canonicalBase . ($_hasChoice ? '?a=' . $quizChoice : '');
    $_robotsNoindex = true;
}
...
// 本文（explanation_body）を組み立てる。AutoLinkはindexableな正規版（?aなし）にのみ適用する。
ob_start();
?>
<p class="quiz-kaisetsu-text"><?= nl2br(htmlspecialchars($quiz['explanation_body'], ENT_QUOTES, 'UTF-8')) ?></p>
<?php
$_bodyHtml = ob_get_clean();
if ($_isIndexable && !$_hasChoice) {
    require_once __DIR__ . '/../inc/auto-link.php';
    $_bodyHtml = autoLink($_bodyHtml, $quiz['slug']);
}
?>
<!DOCTYPE html>
<html lang="ja">
...
.al-link{color:var(--violet-lt);text-decoration:underline;text-decoration-style:dotted;text-underline-offset:3px}
