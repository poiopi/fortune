...
function jobClass(y,m,d){
  const entries=Object.entries(sc).sort((a,b)=>b[1]-a[1]);
  const top=entries[0]?entries[0][0]:'free';
  const seed=((y*10000+m*100+d)*2654435761)>>>0;

  const JOBS={
    battle:[
      {emoji:'⚔️', name:'勇者',       en:'ヒーロー',       desc:'困難に立ち向かう強さで周囲を引っ張るリーダー。',              tags:['力強さ','勇気','行動力'],  proph:'試練を超えるたびに真の輝きを放つでしょう。'},
      {emoji:'🛡️', name:'聖騎士',     en:'パラディン',     desc:'光の力と剣技を併せ持つ護りの戦士。正義を信じる不屈の心を持つ。',tags:['守護','正義','不屈'],      proph:'守るべきものが、あなたに無限の力を与えるでしょう。'},
      {emoji:'🖤', name:'暗黒騎士',   en:'ダークナイト',   desc:'闇の力を操り戦う異端の騎士。誰も踏み込まぬ道を一人で切り開く。',tags:['孤高','力','覚悟'],        proph:'暗闇の中でこそ、あなたの光は最も眩しく輝くでしょう。'},
      {emoji:'👊', name:'武闘家',     en:'モンク',         desc:'己の肉体を極限まで鍛え上げた格闘の達人。武器より己の意志が最強の盾。',tags:['鍛錬','精神力','己超越'],  proph:'限界を超えた先に、あなただけの境地が待っています。'},
      {emoji:'⛩️', name:'侍',         en:'サムライ',       desc:'一刀に魂を宿す東洋の剣士。武士道と誇りが全ての根幹をなす。',    tags:['誇り','武士道','集中'],    proph:'刀に宿るあなたの魂が、やがて歴史に名を刻むでしょう。'},
      {emoji:'🔥', name:'狂戦士',     en:'バーサーカー',   desc:'怒りと本能を力に変える戦闘の化身。戦場でこそ最も輝く爆発力を持つ。',tags:['爆発力','本能','突破'],    proph:'誰も止められない勢いが、道を切り開くでしょう。'},
    ],
    magic:[
      {emoji:'🔮', name:'魔法使い',   en:'ウィザード',     desc:'知識と魔力を操る者。隠された真実を見抜く洞察力を持つ。',        tags:['知性','神秘','探求'],      proph:'知識の深みに宿る光があなたを照らすでしょう。'},
      {emoji:'🌑', name:'黒魔道士',   en:'ブラックメイジ', desc:'破壊的な魔法を操る漆黒の術士。その力は諸刃の剣だが使いこなせば無双。',tags:['破壊力','集中','孤独'],    proph:'秘めた力が解放されるとき、世界が震えるでしょう。'},
      {emoji:'🎩', name:'赤魔道士',   en:'レッドメイジ',   desc:'魔法と剣を両立させる万能の魔法剣士。型にはまらない自由な発想が武器。',tags:['万能','柔軟','独自'],      proph:'二つの道を歩むあなたに、二倍の奇跡が訪れます。'},
      {emoji:'💀', name:'死霊術師',   en:'ネクロマンサー', desc:'生と死の狭間を操る禁断の術者。誰も触れぬ領域に答えを見つける。',  tags:['禁断','洞察','超越'],      proph:'タブーを恐れぬ者だけが到達できる真実があなたを待っています。'},
      {emoji:'✴️', name:'魔法戦士',   en:'スペルソード',   desc:'魔法を纏った剣で戦う複合型戦士。知性と力を統合させた次世代の戦い方。',tags:['融合','応用','スマート'],  proph:'二つの力が一つになるとき、最強の扉が開くでしょう。'},
    ],
    wisdom:[
      {emoji:'📚', name:'賢者',       en:'セージ',         desc:'深い知恵と冷静な判断力で仲間を導く存在。',                      tags:['叡智','洞察','冷静'],      proph:'知恵の言葉が多くの命を救うでしょう。'},
      {emoji:'🌟', name:'占星術師',   en:'アストロロジャー',desc:'星々の動きを読み取り運命を導く予言者。宇宙の言語を解読する能力を持つ。',tags:['予言','宇宙','読解'],    proph:'星が示すあなたの道は、想像よりはるかに輝いています。'},
      {emoji:'🧭', name:'風水師',     en:'ジオマンサー',   desc:'大地の気と流れを読み解く自然の哲学者。場の力を最大限に活用する達人。',tags:['環境','調和','読み'],      proph:'あなたが立つ場所に、必ず良い気が集まるでしょう。'},
      {emoji:'⚗️', name:'錬金術師',   en:'アルケミスト',   desc:'物質と知識を変換する探求者。不可能を可能に変える変革の化身。',    tags:['変換','探求','発明'],      proph:'あなたの手が触れるもの全てが、黄金に変わっていくでしょう。'},
      {emoji:'💙', name:'青魔道士',   en:'ブルーメイジ',   desc:'敵の技を習得し自分のものにする学習の天才。吸収力と応用力で世界を驚かせる。',tags:['学習','吸収','応用'],  proph:'出会う全ての経験が、あなたの力になっていくでしょう。'},
    ],
    agility:[
      {emoji:'🗡️', name:'盗賊',       en:'シーフ',         desc:'素早さと機転で道を切り開く天才的な観察眼を持つ。',              tags:['俊敏','機転','自由'],      proph:'誰も見えない道を切り開いていくでしょう。'},
      {emoji:'🌙', name:'暗殺者',     en:'アサシン',       desc:'影に潜み一瞬で勝負を決める究極の狙撃者。存在を消す技術が唯一無二の強み。',tags:['精密','隠密','決定力'],  proph:'最高のタイミングを見極める眼が、必勝をもたらします。'},
      {emoji:'🥷', name:'忍者',       en:'ニンジャ',       desc:'幻術と体術を駆使する影の申し子。風のように現れ霧のように消える。',  tags:['幻術','瞬発','変幻'],      proph:'風のように自由なあなたを、誰も縛ることはできないでしょう。'},
      {emoji:'🏹', name:'狩人',       en:'レンジャー',     desc:'遠距離から正確に仕留める野生の名手。自然と共に生きる観察眼が武器。',tags:['精度','観察','忍耐'],      proph:'遠い目標をも、あなたの矢は必ず射抜くでしょう。'},
      {emoji:'🔫', name:'銃士',       en:'ガンナー',       desc:'銃火器を操る近代戦の先駆者。冷静な判断と速射が唯一無二のスタイル。',tags:['冷静','速射','判断'],      proph:'決断の速さが、あなたを最前線に立たせるでしょう。'},
    ],
    heal:[
      {emoji:'🙏', name:'僧侶',       en:'プリースト',     desc:'祈りと癒しの力で仲間を守る光の存在。深い信仰心が最大の武器。',    tags:['癒し','信仰','献身'],      proph:'あなたの祈りが、世界に平和をもたらすでしょう。'},
      {emoji:'🌸', name:'白魔道士',   en:'ホワイトメイジ', desc:'聖なる魔法で仲間の傷を癒す純白の術士。その優しさは最強の盾となる。',tags:['回復','純粋','守護'],      proph:'あなたの優しさが積み重なって、奇跡を起こすでしょう。'},
      {emoji:'🌀', name:'召喚士',     en:'サモナー',       desc:'異界の存在を呼び出す契約の魔術師。見えない縁を力に変える唯一の使い手。',tags:['縁','契約','召喚'],      proph:'あなたが呼び寄せる縁こそが、最強の武器となるでしょう。'},
      {emoji:'⏳', name:'時魔道士',   en:'タイムメイジ',   desc:'時間を操る禁断の力を持つ時空の番人。過去と未来を同時に見通す叡智者。',tags:['時間','洞察','支配'],    proph:'時の流れを味方にするとき、あなたは無敵になるでしょう。'},
    ],
    free:[
      {emoji:'🎵', name:'吟遊詩人',   en:'バード',         desc:'自由な発想と語りで人々の心を動かす。縛られない精神が武器。',      tags:['自由','創造','共感'],      proph:'あなたの物語が世界を動かすでしょう。'},
      {emoji:'💃', name:'踊り子',     en:'ダンサー',       desc:'舞いで仲間を鼓舞し敵を惑わす舞踏の達人。身体が奏でるリズムが最高の魔法。',tags:['表現','魅了','リズム'],  proph:'あなたが踊るとき、世界が一緒に動き出すでしょう。'},
      {emoji:'🎭', name:'物まね師',   en:'マイム',         desc:'何者にでもなれる千変万化の演者。見たものを完璧に再現する天才的な模倣力。',tags:['適応','演技','コピー'],  proph:'全てを吸収するあなたは、やがて唯一無二の存在になるでしょう。'},
      {emoji:'🐉', name:'竜騎士',     en:'ドラグーン',     desc:'龍と心を通わせ空を駆ける天空の戦士。高い視点と跳躍力が他を圧倒する。',tags:['飛翔','龍の力','視野'],  proph:'空高く飛ぶほど、あなたの道が拓けるでしょう。'},
    ],
    nature:[
      {emoji:'🌿', name:'薬師',       en:'ハーバリスト',   desc:'自然の力と命の流れを理解する者。地に足のついた知恵で癒す。',      tags:['自然','癒し','智慧'],      proph:'自然との絆があなたに無限の力を与えるでしょう。'},
      {emoji:'🦁', name:'魔物使い',   en:'ビーストマスター',desc:'野生の魔物と心を通わせる異能者。誰も近づけぬ獣をも従わせる不思議な力。',tags:['共存','野生','絆'],      proph:'あなたの心に触れたものは、全て味方になっていくでしょう。'},
      {emoji:'🔨', name:'鍛冶師',     en:'ブラックスミス', desc:'炎と金属を操り最強の装備を生み出す職人。一打一打に魂を込める職人気質。',tags:['鍛造','忍耐','魂'],        proph:'あなたが丹精込めて作るものが、誰かの命を救う日が来るでしょう。'},
      {emoji:'💰', name:'商人',       en:'マーチャント',   desc:'人と物の流れを読み価値を生み出す経済の達人。縁と情報が最大の資本。',tags:['交渉','縁','目利き'],      proph:'あなたの目利き力が、眠れる価値を掘り起こすでしょう。'},
      {emoji:'🪆', name:'人形使い',   en:'パペッティア',   desc:'精巧な人形を操る謎めいた術師。誰も気づかぬ糸を引き、舞台を演出する。',tags:['演出','謀略','精密'],      proph:'見えない糸を巧みに操るあなたが、物語の真の主人公です。'},
    ],
  };

  const pool=JOBS[top]||JOBS.free;
  const job={...pool[seed%pool.length]};

  const PREFIX_SERIOUS=['伝説の','無敵の','真の','覚醒した','運命の','鋼の意志を持つ','魂を燃やす','神に選ばれし','勇敢な','勢いのある','誇り高き','前途有望な','生まれながらの','類まれなる','時代を超えた'];
  const PREFIX_GAG=['去年まで会社員だった','実は','思い込みの激しい','本人だけ知らない','自称','うっかり選ばれた','なぜか','気づいたら','一週間前に転職した','周りがそう呼ぶ','書類上は','師匠も首をかしげた','スキルポイントを間違えた','転職希望の','本当は農家になりたかった'];
  const PROPH_PRE=['星が語りかけています──','魂の記録に刻まれています──','運命の書にはこう記されています──','古の予言に曰く──','天空の意志が示すには──','あなたの魂が知っています──','神託によれば──','宇宙の摂理が告げています──','魔法石が輝きながら伝えています──','運命の羅針盤が指し示すは──'];

  const pfxPool=(seed%10)<7?PREFIX_SERIOUS:PREFIX_GAG;
  const pfx=pfxPool[Math.floor(seed/10)%pfxPool.length];
  const prophPre=PROPH_PRE[(seed>>8)%PROPH_PRE.length];

  job.name=pfx+job.name+'（'+job.en+'）';
  job.proph=prophPre+job.proph;
  return job;
}
