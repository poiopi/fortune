...
/**
 * generate-shichu-trait-mapping-doc.php
 *
 * inc/shichu-trait-mapping.php（単一の情報源）から docs/shichu-trait-mapping.md を生成する。
 * tarotと同じ方式。手で表を書き写すとdocsと実装がずれるため、常にこのスクリプトで再生成する。
 *
 * 実行方法: php tests/tools/generate-shichu-trait-mapping-doc.php
 */
...
foreach (SHICHU_TRAIT_MAPPING['dayStrength'] as $label => $rules) {
    foreach ($rules as $rule) {
        $rows[] = "| {$rule['id']} | dayStrength | {$label} | {$rule['trait']} | +{$rule['score']} |";
    }
}
foreach (SHICHU_TRAIT_MAPPING['tenGod'] as $idx => $rules) {
    foreach ($rules as $rule) {
        $rows[] = "| {$rule['id']} | tenGodGrid | {$rule['keyword']} | {$rule['trait']} | +{$rule['score']} |";
    }
}
...
$md .= "| Rule ID | raw項目 | 条件 | trait | score |\n";
$md .= "|---|---|---|---|---:|\n";
$md .= implode("\n", $rows) . "\n";
...
file_put_contents(__DIR__ . '/../../docs/shichu-trait-mapping.md', $md);
echo "Wrote docs/shichu-trait-mapping.md (" . count($rows) . " rows)\n";
