...
6. **Trait Mapping Table作成**：`inc/{占術}-trait-mapping.php`（PHP配列。単一の情報源）として先に仕様を確定する。既存コンテンツの意味をtraitへ写像するだけで、新しい占術解釈は加えない
   - スコア基準：+2=中心的テーマ、+1=副次的テーマ。**スコアは、その入力（型・カード・十神等）を説明する際の「中心的特徴」か「副次的特徴」かを表すものであり、Traitの強さや優劣を表すものではない。異なる型・カード間で総得点を比較する目的には用いない**（例：ある型の総得点が他の型より高くても、その型が「勝っている」ことを意味しない）
