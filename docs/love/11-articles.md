...
MBTIの受けリンクが突出しているのは、Style/Tendency/Seiza/Blood/Bundleの各記事が「MBTI16タイプとの相関」という16行のランキング表を持つため（9カテゴリ×16行≒480件がここに集中）。編集判断による偏りではなく、MBTIの選択肢数（16）に起因する構造的な結果である。Tendency（56）・Blood（60）が少ないのは、項目数がそれぞれ2・4と少なく、他カテゴリの相関表の対象になる機会自体が少ないため。今後の分析では、リンク数だけでなく「何ページから受けているか（分散度）」も併せて見ることが望ましい。
