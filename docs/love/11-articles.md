...
### 7-2. カテゴリ別内部リンク数（ベースライン記録）

将来「回遊率が悪い」等の問題が出た際の比較基準として、v1.2完成時点（2026-07-12）のカテゴリ別内部リンク数を記録する。60ページ（52記事＋7カテゴリハブ＋ルートハブ）を実際にfetchし、レンダリング後の実リンクを集計した実測値。

| カテゴリ | 受ける内部リンク（総数） | 受けリンク元ページ数（全60中） | 1ページあたり平均 | 出す内部リンク |
|---|---:|---:|---:|---:|
| MBTI | 630 | 51 | 12.4 | 286 |
| Style | 212 | 51 | 4.2 | 231 |
| Bundle | 153 | 48 | 3.2 | 156 |
| Guide | 99 | 55 | 1.8 | 79 |
| Seiza | 69 | 22 | 3.1 | 310 |
| Blood | 60 | 19 | 3.2 | 138 |
| Tendency | 56 | 30 | 1.9 | 71 |

MBTIの受けリンクが突出しているのは、Style/Tendency/Seiza/Blood/Bundleの各記事が「MBTI16タイプとの相関」という16行のランキング表を持つため（9カテゴリ×16行≒480件がここに集中）。編集判断による偏りではなく、MBTIの選択肢数（16）に起因する構造的な結果である。Tendency（56）・Blood（60）が少ないのは、項目数がそれぞれ2・4と少なく、他カテゴリの相関表の対象になる機会自体が少ないため。

「受けリンク元ページ数」を見ると別の側面が分かる。MBTIとStyleはリンク元ページ数が同じ（51/60ページ）だが、MBTIは1ページあたり平均12.4本（16行の表がまるごと1セット）、Styleは平均4.2本であり、MBTIの高さは「濃さ」でありStyleと「広さ」は同等である。Guideは最も広く（55/60ページ）薄く（平均1.8本）リンクされており、`kekka-no-mikata`等への単発リンクがrelatedItemsに分散配置されている設計通りの結果。Blood（19ページ）・Seiza（22ページ）はリンク元が最も狭く、将来これらのカテゴリでCTRが伸び悩んだ場合は「リンク元ページを増やす」方が「1ページあたりのリンク数を増やす」より効果的である可能性が高い。
